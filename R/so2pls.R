...
#'
#' Computes the optimal number of features/groups to keep for each joint component for an sO2PLS run. Directly
#' copied from the \code{\link[OmicsPLS]{crossval_sparsity}} function, but improved the output for plotting purposes.
#'
#' @param omicspls_input A named list of length 2, produced by \code{\link{get_input_omicspls}}.
#' @param n Integer, number of joint PLS components. Must be positive.
#' @param nx Integer, number of orthogonal components in `X`. Negative values are interpreted as 0.
#' @param ny Integer, number of orthogonal components in `Y`. Negative values are interpreted as 0.
#' @param nr_folds integer, number of folds for the cross-validation. Default value is 10.
#' @param keepx_seq Numeric vector, how many features/groups to keep for cross-validation in each of the
#' joint components of `X`. Sparsity of each joint component will be selected sequentially.
#' @param keepy_seq Numeric vector, how many features/groups to keep for cross-validation in each of the
#' joint components of `Y`. Sparsity of each joint component will be selected sequentially.
#' @param groupx Character vector, group name of each `X`-feature. Its length must be equal to the number
#' of features in `X`. The order of the group names must corresponds to the order of the features.
#' If `NULL`, no groups are considered. Default value is `NULL`.
#' @param groupy Character vector, group name of each `Y`-feature. Its length must be equal to the number
#' of features in `Y`. The order of the group names must corresponds to the order of the features.
#' If `NULL`, no groups are considered. Default value is `NULL`.
#' @param tol Numeric, threshold for which the NIPALS method is deemed converged. Must be positive.
...
#' joint components of \code{X} and \code{Y} (elements \code{x1, y1, x2, y2}, etc),
#' and the number of features to keep from \code{X} and \code{Y} yielding the highest covariance under the
#' 1 standard error rule (elements \code{x_1sd1, y_1sd1, x_1sd2, y_1sd2}, etc).
#' \item \code{Covs}: a list, with as many elements as number of joint components (\code{n}).
#' Each element is a matrix giving the average covariance between the joint components of \code{X} and \code{Y}
#' obtained across the folds, for each tested values of \code{keepx} (columns) and of \code{keepy} (rows).
#' \item \code{SEcov}: a list, with as many elements as number of joint components (\code{n}).
#' Each element is a matrix giving the standard error of the covariance between the joint components of \code{X} and \code{Y}
#' obtained across the folds, for each tested values of \code{keepx} (columns) and of \code{keepy} (rows).
#' }
#' @export
so2pls_crossval_sparsity <- function(omicspls_input, n, nx, ny, nr_folds = 10, keepx_seq = NULL, keepy_seq = NULL,
                                     groupx = NULL, groupy = NULL, tol = 1e-10, max_iterations = 100) {
  if (!is.list(omicspls_input) | length(omicspls_input) != 2) stop("'omicspls_input' should be a list of length 2 produced by get_input_omicspls().")
