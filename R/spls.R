.get_multilevel <- function(spls_input, mo_data, multilevel) {
  if (is.null(mo_data)) stop("Need to provide 'mo_data' argument to use the 'multilevel' option.")

  if (!(length(multilevel) %in% c(1, 3))) {
    stop("'multilevel' argument should be of length 1 (for one-factor decomposition) or 3 (for two-factor decomposition).")
  }
...
#'
#' Creates an object that can be used as input for the (s)PLS functions from the mixOmics package.
#' It contains the two omics datasets selected, restricted to common samples.
#'
...
#' [mixOmics::spls()] enables one- and two-factor decomposition. For one-factor decomposition,
...
#' @param mode Character, the mode of PLS to use of the analysis (see [sPLS documentation](http://mixomics.org/methods/spls/)).
#' Should be one of `'regression'`, `'canonical'`, `'invariant'` or `'classic'`.
...
#' See Details.
#' @returns A list, in which each element corresponds to one omics dataset, with samples as rows and features as columns.
#' The mode to use for the analysis is stored in the `mode` attribute of the returned object.
#' @export
get_input_spls <- function(mo_data, mode, datasets = names(mo_data), multilevel = NULL) {

  if (length(datasets) != 2) stop("'datasets' argument: expecting a vector of length 2 (as (s)PLS integrates 2 datasets).")
  .check_names(
    mode,
...
#' Run sPLS algorithm
#'
#' Runs the sPLS algorithm (\code{\link[mixOmics]{spls}}) from the \code{mixOmics} package.
#'
#' @param spls_input A \code{mixOmics} input object created with \code{\link{get_input_spls}}.
#' @param ... Arguments passed to [mixOmics::spls()].
#' @returns An object of class `mixo.spls` (if `keepX` and/or `keepY` arguments were provided)
#' or `mix.pls` (if they were not), see [mixOmics::spls()] and [mixOmics::pls()].
#' @export
...
#' cross-validation results obtained with [mixOmics::perf()] on a PLS or sPLS result,
#' using the the mean `Q2.total` values. Note that this function is experimental,
#' the corresponding diagnostic plots should be considered when selecting the
#' optimal number of components to use.
#'
#' The selection is made as follows:
#' \itemize{
#' \item If all `Q2` values are below the threshold specified with `thr`, the
#' number of components yielding the highest `Q2` value is selected.
#' \item If all `Q2` values are above the threshold, the number of components
#' yielding the lowest `Q2` value is selected.
#' \item If the `Q2` values are increasing, the number of components `n` is
#' selected such that `n+1` is the smallest number of components with a `Q2`
#' value above the threshold.
#' \item If the `Q2` values are decreasing, the number of components `n` is
#' selected such that `n+1` is the smallest number of components with a `Q2`
...
#' @returns An integer, the optimal number of components to use for the sPLS run.
#' @export
spls_get_optim_ncomp <- function(spls_perf, thr = 0.0975, min_ncomp = 1) {
  ...
  if (all(q2_vals < thr)) {
    ...
    ## 1. if the Q2 values decrease, we want to retain the highest comp value with
    ##    a Q2 value above the threshold
    res1 <- which(q2_vals < thr)[1] - 1

    ## 2. if the Q2 values increase, we want to retain the highest comp value with
    ##   a Q2 value below the threshold
...
#' `seq(5, 30, 5)` will be used, truncated to only retain
#' values that are inferior or equal to the number of features in the X or Y
#' dataset.
#'
#' @param spls_input A \code{mixOmics} input object created with \code{\link{get_input_mixomics_unsupervised}}.
#' @param keepX Numeric vector, values for the number of features to retain from dataset X
#' to test. Default value is `NULL` (default sequence of values will be used, see details).
#' @param keepY Numeric vector, values for the number of features to retain from dataset Y
#' to test. Default value is `NULL` (default sequence of values will be used, see details).
#' @param cpus Integer, the number of CPUs to use when running the code in parallel. For advanced users,
#' see the \code{BPPARAM} argument of \code{\link[mixOmics]{tune.spls}}.
#' @param ... Further arguments passed to \code{\link[mixOmics]{tune.spls}}.
#' @return A list (see \code{\link[mixOmics]{tune.spls}}).
#' @export
...
#'
#' Displays the results of cross-validation to tune the number of components to retain in each dataset
#' for a sPLS run. Similar to \code{\link[mixOmics]{plot.tune.spls}}.
#'
#' The plot displays the correlation or RSS between the latent components obtained with the corresponding values
#' of `keepX` (x-axis) and `keepY` (y-axis) and the latent components of the full model (i.e. that retains all features).
#' The colour of the points shows the mean correlation/RSS across the cross-validation folds, and the size of the points'
#' shadow (in gray) represents the coefficient of variation (COV) of the correlation/RSS, i.e. standard error divided by mean.
#' The point corresponding with the optimal value for `keepX` and `keepY` is indicating with a red border.
#'
#' @param spls_tune_res The result of \code{\link{spls_tune}} or \code{\link[mixOmics]{tune.spls}}.
#' @return A ggplot.
#' @export
spls_plot_tune <- function(spls_tune_res) {
  ...
  ggplot2::ggplot(df, aes(x = keepX, y = keepY)) +
    ggplot2::geom_point(aes(size = cov, colour = optimum.keepA), shape = 21, fill = "gray70", colour = "gray60") +
    ggplot2::geom_point(aes(fill = mean, colour = optimum.keepA), size = 3, shape = 21) +
    ggplot2::scale_colour_manual(values = c("FALSE" = "black", "TRUE" = "red"), guide = "none") +
...
#'
#' Extracts the `ncomp`, `keepX` and `keepY` parameters from a sPLS run and format them into a table.
#'
#' @param spls_res The output from \code{\link[mixOmics]{spls}} or \code{\link{spls_run}}.
#' @return A tibble.
#' @export
spls_get_params <- function(spls_res) {
  tibble::tibble(
    ...
    Description = c(
      ...
      "Number of features retained in dataset X for each latent component",
      "Number of features retained in dataset Y for each latent component"
    ),
    Value = c(
...
#'
#' Computes the average sample coordinates for sPLS components
#' across the two datasets.
#'
#' @param spls_res The output from the [spls_run()] or [mixOmics::spls()] function.
