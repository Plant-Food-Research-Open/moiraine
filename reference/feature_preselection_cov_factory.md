...
- `cov_spec`: a target that generates a grouped tibble where each row
...
- `cov_mat`: a dynamic branching target that run the
...
- `individual_cov_values`: a dynamic branching target that runs the
...
- `filtered_set_cov`: a target to retain from the original
  `MultiDataSet` object only features selected based on their COV
  values.
