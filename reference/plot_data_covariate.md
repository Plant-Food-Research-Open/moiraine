...
For a given set of features, plots their value against a sample
covariate from the samples metadata. Depending on whether the covariate
is continuous or discrete, will generate either a scatterplot or a
violin plot.
...
- samples:
  ...
  (default), all samples in the corresponding dataset will be used.
...
- colour_by:
  ...
  Character, name of column in one of the samples metadata tables from
  `mo_data` to use to colour the observations in the plot. Default value
  is `NULL`.

- shape_by:

  Character, name of column in one of the samples metadata tables from
  `mo_data` to use as shape for the observations in the plot.
